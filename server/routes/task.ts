...
router.post("/create-task", async (req: Request, res: Response) => {
  try {
    ...
    const newTask = await pool.query("SELECT create_task($1) as task_id", [description]);
...
router.get("/get-tasks", async (req: Request, res: Response) => {
  try {
    const allTasks = await pool.query("SELECT * FROM get_all_tasks()");
...
router.get("/get-task/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const task = await pool.query("SELECT * FROM get_task_by_id($1) AS task(task_id INTEGER, description TEXT)", [id]);
...
router.put("/update-task/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { description } = req.body;
    const updateTask = await pool.query("SELECT update_task($1, $2) AS message", [id, description]);
...
router.delete("/delete-task/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const deleteTask = await pool.query("SELECT delete_task($1) as message", [id]);
